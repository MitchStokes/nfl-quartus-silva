import { GameLine } from './SeasonSim';
...
export function simulateSyntheticParlayBetting(gameLines: GameLine[], totalWinLines: TotalWinLine[], teamName: string) {
  ...
  gameLines.forEach((game) => {
    ...
    if (Math.random() <= team1NormedProb) {
      ...
      if (team1BetAmount) bank = team1BetAmount * (1 / team1NormedProb) * 0.975;
      if (game.team1 == teamName) teamWins++;
    } else {
      // Team 2 won
      const ratingProportion = curRatings[game.team2] / curRatings[game.team1];
      curRatings[game.team1] -= k / ratingProportion;
      curRatings[game.team2] += k / ratingProportion;
      if (team2BetAmount) bank = team2BetAmount * (1 / team2NormedProb) * 0.975;
      if (game.team2 == teamName) teamWins++;
    }
